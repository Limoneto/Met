-- MET — Seed de datos demo para MySQL

-- 1. RBAC: Roles
INSERT INTO rol (id, codigo, nombre) VALUES
  (UUID(), 'admin', 'Administrador'),
  (UUID(), 'contador', 'Contador'),
  (UUID(), 'empleado', 'Empleado'),
  (UUID(), 'cliente', 'Cliente');

-- 2. RBAC: Permisos (minimal set)
INSERT INTO permiso (id, codigo, descripcion) VALUES
  (UUID(), 'ver_socios', 'Ver listado de socios'),
  (UUID(), 'crear_socio', 'Crear nuevo socio'),
  (UUID(), 'ver_empleados', 'Ver empleados'),
  (UUID(), 'ver_salud', 'Ver datos de salud'),
  (UUID(), 'ver_finanzas', 'Ver datos financieros'),
  (UUID(), 'reportes', 'Generar reportes'),
  (UUID(), 'configurar', 'Configurar sistema');
